myApp.directive('compareBarGsvpChart', function($http) {
  return {
    restrict: 'AE',
    scope: {
      data: '=',
    ...
    link: function(scope) {
      scope.$watch('data', function(newVal) {
        ...
        if (dataset) {
          ...
          // ternary operator to determine if Claims_info or Apps has a larger scale
          var yScale = d3.scale.linear()
            .domain([0, d3.max(dataset, function(d) {
              return (d[barData1] > d[barData2]) ? d[barData1] : d[barData2];
            })])
            .range([h, 0]);
          ...
          function updateBars() {
            svg.selectAll(".Docs_Forms").remove();
            svg.selectAll(".Claims_info").transition().duration(500).attr("width", xScale.rangeBand());
          }
